...
## ⚠️ 最優先: 自動改善ループ（指示書233）

Claude Code は毎回この3点を必ず実行する。**作業を始める前に読み、作業の途中で書く。**

### 1. 作業開始前に `RULES.md` を全文読む
リポジトリ直下の [`RULES.md`](./RULES.md) が累積行動ルール台帳（R-NN）。過去の手戻り・バグ・指摘を抽象化した恒久ルールが入っている。
便の冒頭で `npm run rules:lint` を実行し、重複・矛盾の候補が出ていないか確認する。

### 2. 手戻り・バグ・指摘が起きたら `RULES.md` に追記する
その便のうちに、該当ルールを**追記または既存ルールの更新**として反映する（形式は RULES.md 冒頭に記載）。
番号は追記のみで、欠番になっても再利用しない。抽象化した結論だけを置き、生ログは 3. に書く。

### 3. 失敗トレースを `~/Downloads/lumina_traces/YYYYMMDD.md` に追記する
リポジトリ**外**。便ごとの失敗・手戻り・想定外を1行1件で記録する。書式が崩れないよう、追記はスクリプト経由で行う。
...
- 「抽象化したルール」列には `R-NN`（登録済み）または `未登録`（＋理由）を書く
- **個人情報・生成本文は書かない**（識別子と事象のみ）
- 蓄積が溜まったら、次の handover 作成時に「学習の要約」（ファイル末尾のセクション）として反映する
...
- 自動修正・自動分類等は「勝手にDB保存しない／ユーザーを待たせない」
  ...
  - 例外（247・院長指示）: 📝テキスト分析 と 🔭ディープリサーチ の**生成結果のストック保存**だけは
    既定で自動保存する。対象は必ずこの2画面に限り、`lib/auto-stock-save.ts` の設定（🎛表示設定）で
    OFFにできること・保存済み表示・二重保存の防止・失敗時の手動再試行をセットで満たすこと（R-53）
