...
const TERMS: Term[] = [
  {
    ...
    detail: '大量のデータを学習することで、文章を書いたり、画像を認識したり、質問に答えたりできるようになります。xLUMINAもAIを使って情報収集や文章作成を行っています。',
  ...
  },
  {
    ...
    detail: 'プロンプトの書き方によって、AIの回答の質が大きく変わります。具体的で明確な指示を出すほど、良い結果が得られます。xLUMINAでは各機能が最適なプロンプトを自動で設定しています。',
  ...
  },
  {
    ...
    example: 'xLUMINAはClaude（Anthropic社のLLM）を使って文章生成や分析を行っています。',
  ...
  },
  {
    ...
    detail: 'Application Programming Interface の略。xLUMINAはAnthropicのAPIを通じてClaudeと通信し、PerplexityのAPIで最新情報を検索しています。APIキーは「入館証」のようなもので、権限のある人だけがサービスを使えます。',
    example: 'xLUMINAが「AIで文章を生成する」とき、裏側でAnthropicのAPIを呼び出しています。',
  ...
  },
  {
    ...
    detail: '日本語では1文字≒1〜2トークン程度。AIへの入力と出力の合計トークン数で料金が決まります。xLUMINAのWeb情報収集で「回答の長さ」を選べるのは、このトークン数を調整しているためです。',
  ...
  },
  {
    ...
    detail: 'Retrieval-Augmented Generation（検索拡張生成）の略。AIの知識は学習時点で止まっているため、最新情報をリアルタイムで検索して回答に組み込む仕組みです。xLUMINAのWeb情報収集機能がRAGの一種です。',
    example: 'xLUMINAが「2026年のAIトレンド」を検索して回答するとき、RAGの仕組みを使っています。',
  ...
  },
  {
    ...
    detail: 'Strengths（強み）・Weaknesses（弱み）・Opportunities（機会）・Threats（脅威）の頭文字。新規事業の検討や競合分析に広く使われます。xLUMINAのAI分析エンジンで自動生成できます。',
  ...
  },
  {
    ...
    detail: 'Mission（使命）・Vision（将来像）・Values（価値観）の略。会社の方向性を全員で共有するための羅針盤です。xLUMINAの経営インテリジェンス機能でAIがMVVの策定を支援します。',
  ...
  },
  {
    ...
    detail: 'AIを使って論文の重要度ランキングや関連論文の推薦を行う次世代の学術検索エンジン。xLUMINAの文献検索機能はこのAPIを使用しています。医学・工学・経営など全分野に対応。',
    example: 'AI倫理に関する最新の研究論文を探したいとき、xLUMINAの文献検索から検索できます。',
  ...
  },
  {
    ...
    detail: 'xLUMINAはVercelを使ってインターネットに公開されています。GitHubにコードをプッシュするだけで自動的にデプロイ（公開）される仕組みになっています。',
  ...
  },
  {
    ...
    category: 'ツール',
    simple: 'xLUMINAのユーザーデータを保存しているクラウドデータベース。',
    detail: 'ユーザーのアカウント情報・リサーチ保存のデータ・下書きなどがNeon上に保存されています。シンガポールのサーバーを使用。PostgreSQLはデータを整理して保存するための仕組みの名前です。',
    example: 'リサーチ保存に追加した調査結果は、Neonデータベースに記録されているため、次回ログイン時も表示されます。',
  ...
  },
  {
    ...
    reading: 'すとらいぷ',
    category: 'ツール',
    simple: 'オンライン決済を簡単に実装できるサービス。xLUMINAの有料プラン決済に使用。',
    detail: 'クレジットカード決済の処理をStripeが代行します。現在はテストモードで動作中。本番モードに切り替えると実際の決済が可能になります。月額¥2,980のProプランの決済に使用予定。',
    example: 'xLUMINAの「Proにアップグレード」ボタンを押すとStripeの決済画面に移動します。',
  ...
  },
  {
    ...
    reading: 'ぱーぷれきしてぃ',
    category: 'ツール',
    simple: 'リアルタイムでWeb検索ができるAI検索エンジン。xLUMINAの情報収集に使用。',
    detail: 'ただの検索エンジンではなく、検索結果をAIがまとめて回答してくれるサービス。xLUMINAはPerplexityのAPIを使って最新のWeb情報を収集しています。',
    example: 'xLUMINAのWeb情報収集でリアルタイムの市場情報を取得するときに使われています。',
  ...
  // ===== 技術スタック =====
  {
    ...
    simple: 'xLUMINAの全ページを作るのに使っているWeb開発の「道具箱」。',
    detail: 'Reactというライブラリをベースに、ページ表示の高速化・SEO対策・APIの作成まで一つのツールで対応できます。xLUMINAでは画面（フロントエンド）とAPIサーバー（バックエンド）の両方をNext.jsで構築しています。',
    example: 'ダッシュボード・Web情報収集・文章作成など、xLUMINAの全ページがNext.jsで作られています。またAIへのリクエストを処理するAPI（/api/websearch など）もNext.jsのAPI Routesで実装しています。',
  ...
  },
  {
    ...
    detail: '変数や関数に「これは数字」「これは文字列」と型を指定することで、コードの間違いを事前に検出できます。xLUMINAの全コードがTypeScriptで書かれており、開発中のバグを大幅に減らしています。',
  ...
  },
  {
    ...
    detail: '「青いボタン」「余白16px」のようなデザインをクラス名を書くだけで実現できます。xLUMINAのUI（ボタン・カード・レイアウトなど）のスタイリングに使用しています。',
    example: 'xLUMINAの「無料で始める」ボタンの色・角丸・サイズ・ホバー時の変化などが全てTailwind CSSで設定されています。',
  ...
  },
  {
    ...
    simple: 'xLUMINAのAI機能（文章生成・分析・Web検索）の中核となるAIエンジン。',
    detail: 'Anthropic社が開発したClaude AIをAPIで利用しています。文章作成・SWOT分析・Web情報収集・Tips自動生成など、xLUMINAの主要機能のほぼすべてがこのAPIを通じてClaudeと通信しています。使った量（トークン数）に応じて料金が発生します。',
  ...
  },
  {
    ...
    simple: 'ユーザーのデータ（アカウント・リサーチ保存・下書き）を保存するクラウドデータベース。',
    detail: 'PostgreSQLという信頼性の高いデータベースをクラウドで使えるサービスです。xLUMINAではユーザー認証情報・リサーチ保存のデータ・文章の下書きなどを全てNeonに保存しています。シンガポールのサーバーを使用しており、日本からのアクセスも高速です。',
    example: 'リサーチ保存に調査結果を追加すると、そのデータがNeonのデータベースに書き込まれます。次回ログインしても表示されるのは、Neonにデータが永続保存されているためです。',
  ...
  },
  {
    ...
    simple: 'xLUMINAのログイン・ログアウト・セッション管理を担当するライブラリ。',
    detail: 'ユーザーがログインした状態を安全に管理します。「誰がログインしているか」を確認して、本人しか自分のデータにアクセスできないようにしています。パスワードの暗号化やセッション管理を自動で処理してくれます。',
    example: 'xLUMINAにログインするとセッションが作成され、ページを移動してもログイン状態が維持されます。ログアウトするとセッションが削除されます。',
  ...
  },
  {
    ...
    simple: 'xLUMINAをインターネットに公開・運用するためのクラウドサービス。',
  ...
  },
  {
    ...
    simple: 'xLUMINAのProプラン（月額¥2,980）の決済処理を担当するサービス。',
    detail: 'クレジットカード決済・定期課金（サブスクリプション）の処理をStripeが代行します。現在はテストモードで動作中。本番モードに切り替えると実際の決済が可能になります。PCI DSS準拠の高いセキュリティが特徴です。',
    example: 'xLUMINAの「Proにアップグレード」ボタンを押すとStripeの決済画面に移動します。月額¥2,980の定期課金もStripeが自動処理します。',
  ...
  },
  {
    ...
    detail: 'xLUMINAのIntelligence Hubなどでリアルタイムの最新情報を取得するために使用しています。通常の検索エンジンと違い、検索結果をAIが自動でまとめて返してくれるため、情報収集の効率が大幅に上がります。',
  ...
  },
  {
    ...
    simple: '1.38億件以上の学術論文をxLUMINAから検索するためのAPI。',
    detail: 'Allen Institute for AIが提供する学術論文データベースのAPIです。xLUMINAの文献検索機能はこのAPIを使用しており、医学・工学・経営・AI研究など全分野の最新論文を無料で検索できます。論文の引用数・重要度ランキング・関連論文の推薦機能も提供しています。',
  ...
  // ===== フロントエンド =====
  {
    ...
    example: 'xLUMINAのダッシュボードの各カード・ボタン・入力フォームが全てReactコンポーネントとして作られています。',
  ...
  },
  {
    ...
    example: 'xLUMINAのモーダル・セレクトボックス・トースト通知などのUIパーツにshadcn/uiのコンポーネントを活用しています。',
  ...
  },
  {
    ...
    example: 'xLUMINAのテーマ設定（ダーク/ライト）や選択中の言語設定などをZustandで管理すると、全ページで即座に反映できます。',
  ...
  },
  {
    ...
    example: 'xLUMINAのリサーチ保存ページでデータを取得・表示する処理をTanStack Queryで実装すると、ローディング表示・エラー処理が自動化されます。',
  ...
  },
  {
    ...
    example: 'xLUMINAのリサーチ保存のデータを取得するとき、`prisma.library.findMany({ where: { userId } })` のように直感的に書けます。',
  ...
  },
  {
    ...
    example: 'xLUMINAのような Next.js + Neon 構成では、Prismaの代わりにDrizzleを使うとデータベース操作が高速化できます。',
  ...
  // ===== AI・ML =====
  {
    ...
    detail: 'OpenAI・Anthropic・Googleなど複数のAIプロバイダーに対応した統一インターフェース。ストリーミング応答（文字が徐々に表示される）の実装が数行で書けます。xLUMINAのような AIアプリ開発の標準ライブラリになっています。',
  ...
  },
  {
    ...
    detail: '複数のAIモデルやツールを「チェーン」のように繋げて、複雑な処理を自動化できます。「Web検索→要約→レポート生成」のような多段階処理の実装に使われます。xLUMINAのディープリサーチのような機能を構築するのに適しています。',
  ...
  // ===== インフラ・デプロイ =====
  {
    ...
    example: 'xLUMINAをDockerコンテナ化すると、開発環境・テスト環境・本番環境で全く同じ動作を保証できます。',
  ...
  },
  {
    ...
    detail: 'Continuous Integration（継続的統合）とContinuous Deployment（継続的デプロイ）を自動化します。xLUMINAはVercelが自動デプロイを担当していますが、テストの自動実行などにGitHub Actionsを追加できます。',
  ...
  // ===== テスト =====
  {
    ...
    example: '「リサーチ保存への保存が成功したか」「APIが正しいデータを返すか」などをVitestで自動テストすると、リリース前のバグを検出できます。',
  ...
  },
  {
    ...
    example: 'xLUMINAの「Web情報収集→リサーチ保存→確認」という一連の操作をPlaywrightで自動テストすると、リリース前に問題を検出できます。',
  ...
  // ===== 監視・分析 =====
  {
    ...
    example: 'xLUMINAにSentryを導入すると、ユーザーがエラーを報告しなくても、エラー発生時に自動でSlackに通知が届きます。',
  ...
  },
  {
    ...
    example: 'xLUMINAで「ユーザーがどの機能を一番使っているか」「どこで離脱しているか」をPostHogで分析して、UX改善に活かせます。',
  ...
  // ===== UI/UX =====
  {
    ...
    example: 'xLUMINAのTipsカード・用語カードなどのコンポーネントをStorybookに登録すると、デザイン変更の影響範囲を一目で確認できます。',
  ...
  },
  {
    ...
    example: 'xLUMINAの新しいページをFigmaでデザインしてからコーディングすることで、手戻りを防ぎ開発効率が上がります。',
  ...
  // ===== セキュリティ =====
  {
    ...
    example: 'xLUMINAのWeb情報収集フォームで「空のクエリ」「異常に長い文字列」などの不正入力をZodで検証してブロックできます。',
  ...
  },
  {
    ...
    example: 'xLUMINAにResendを導入すると、新規ユーザー登録時のウェルカムメール・定期アラートのメール通知を簡単に実装できます。',
