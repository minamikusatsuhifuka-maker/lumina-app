...
const TERMS: Term[] = [
  ...
  },
  {
    ...
    detail: 'PostgreSQLという信頼性の高いデータベースをクラウドで使えるサービスです。xLUMINAではユーザー認証情報・ライブラリの保存データ・文章の下書きなどを全てNeonに保存しています。シンガポールのサーバーを使用しており、日本からのアクセスも高速です。',
  ...
  },
  {
    ...
    example: 'xLUMINAのライブラリページでデータを取得・表示する処理をTanStack Queryで実装すると、ローディング表示・エラー処理が自動化されます。',
  ...
  },
  {
    ...
    example: 'xLUMINAのライブラリデータを取得するとき、`prisma.library.findMany({ where: { userId } })` のように直感的に書けます。',
  ...
  },
  {
    ...
    example: 'xLUMINAの「Web情報収集→ライブラリ保存→確認」という一連の操作をPlaywrightで自動テストすると、リリース前に問題を検出できます。',
