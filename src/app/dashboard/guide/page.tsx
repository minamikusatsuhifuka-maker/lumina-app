...
const sections: Section[] = [
  {
    ...
    connections: [
      ...
      'すべての機能 → リサーチ保存：調査・分析・文章をすべて一元管理',
    ],
    faqs: [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: '結果をリサーチ保存に追加するか、AI分析エンジンに渡す' },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { text: '分析結果をリサーチ保存に追加→文章作成ページに引き継ぐ' },
    ],
    ...
    connections: [
      ...
      '→ リサーチ保存：分析結果を保存して、定期的に振り返り・更新',
    ],
    faqs: [
      { q: 'どの分析タイプを選べばいいですか？', a: '新規事業や戦略検討ならSWOT、競合を調べるなら競合分析、次の行動を決めたいならアクション分析がおすすめです。' },
      { q: '過去の分析結果と比較できますか？', a: 'リサーチ保存に追加しておけば、時期を変えて同じテーマを再分析し、変化を比較できます。' },
    ],
  },
  {
    id: 'write',
    ...
    steps: [
      ...
      { text: '生成された文章を編集・コピー・リサーチ保存に追加する' },
    ],
    ...
    connections: [
      ...
      '→ リサーチ保存：テンプレートとして保存し、次回以降の文章作成に再利用',
    ],
    faqs: [
  ...
  },
  {
    ...
    steps: [
      { text: '各機能ページの「📚 リサーチ保存に追加」ボタンを押す' },
      { text: 'リサーチ保存ページでタグ・グループを設定して整理する' },
    ...
    cautions: [
      ...
      'リサーチ保存が増えすぎると検索性が下がるので、不要なデータは定期的に整理しましょう',
  ...
  },
  {
    ...
    steps: [
      ...
      { text: '【競合分析】Web情報収集（競合情報収集）→ AI分析エンジン（競合分析）→ 経営インテリジェンス（差別化戦略）→ リサーチ保存（蓄積）' },
      { text: '【採用強化】経営インテリジェンス（採用戦略策定）→ 文章作成（求人票作成）→ AI分析（候補者評価基準）' },
    ],
    tips: [
      '最初は1つのワークフローを完全にマスターしてから、次に進みましょう',
      '各ステップの結果を必ずリサーチ保存に追加すると、後から振り返れます',
...
const FEATURE_GROUPS: CatalogGroup[] = [
  ...
  },
  {
    group: '管理・設定', emoji: '⚙️',
    items: [
      { href: '/dashboard/library', label: 'リサーチ保存', emoji: '📚', desc: '保存物の蓄積・自動カテゴライズ' },
