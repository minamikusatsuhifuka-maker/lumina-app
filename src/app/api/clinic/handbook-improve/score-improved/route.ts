...
以下の【元の文���】と【改善後の文��】を厳密に比較・評価してく��さい。
...
## 評価の観��
1. ��ンプレート「${templateLabel}」の意��が���れだけ実現され���いるか
2. クリニック理念（ティール組織・リードマネジメント・インサイドアウト・先払い哲学）との一致度
3. 元���文章との具体的な違��と改善度合い

## 重要な指示
- ��案の特徴��正直に差別化して評価すること
- スコアは文章の質に応じて50〜95点の範囲��厳密につける��と
- バランス指標は各軸を独立して��価し、必ず異なる値��つける��と（全て同じ値は禁止）
- score_diffは元の文章との実際の改善幅を示すこと

## 出力形式
必ず以下のJSONのみを返してください。��ードブロック不要：

{
  "score": 整数（50〜95）,
  "score_diff": 整数（元文章からの変化。改善なら正、悪化なら負）,
  "comment": "こ��改善案の特徴と評価を2〜3文で具体的に",
  "good_points": ["具���的な良い点1", "具体的な良い点2"],
  "improve_points": ["具体的な改善点1", "具体的な改善点2"],
  "balance": {
    "readability": 読みやすさ（1〜10、��章の流れと理解しやすさ）,
    "agency": 主体性・自律（1〜10、スタッフ��自分で考え動きたくなるか）,
    "specificity": 具体性（1〜10、行動イ��ージが湧くか）,
    "philosophy": 理念一致度（1〜10、クリニック理念との整合性）,
    "warmth": 温かみ（1��10、感情に響く温か��があるか）
  }
}`,
      },
    ],
  ...
  try {
    ...
    return NextResponse.json({
      score: 0,
      score_diff: 0,
      comment: `⚠️ ���点結果のパースに失敗しました。生の返答：${cleaned.slice(0, 200)}`,
      good_points: [],
      improve_points: [],
      balance: {
        readability: 0,
