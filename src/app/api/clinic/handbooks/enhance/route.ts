...
指示に従って書き直した文章のみ返してください。`;
  } else if (mode === 'boss-to-lead') {
    userPrompt = `以下のハンドブックの文章を、ボスマネジメント型の表現からリードマネジメント型に書き直してください。

【現在の文章】
${chapterContent}

【変換ルール】
ボスマネ（変換前）→ リードマネ（変換後）の例：
- 「〜しなければならない」→「〜することで、あなたの○○が満たされます」
- 「禁止」「罰則」→「大切にしていること」「私たちが守りたい価値」
- 「管理します」「指示します」→「一緒に考えましょう」「あなたはどう思いますか」
- 「提供します」（院長→スタッフへの一方向）→「あなたが選択できます」「あなたが主役です」
- 「育成します」→「あなたが自律的に成長できる環境を整えます」
- 「評価します」→「あなたの成長を一緒に確認します」
- 命令形・強制形 → 問いかけ形・選択形・内発的動機形

スタッフが「やらされている」ではなく「自分で選んでいる」と感じられる表現にしてください。
書き直した文章のみ返してください。説明は不要です。`;
  } else if (mode === 'add-question') {
    userPrompt = `以下のハンドブックの章を読んで、章末に追加する「スタッフへの問いかけ」を生成してください。

【現在の文章】
${chapterContent}

【問いかけの要件】
- スタッフが自分ごととして考えられる問いかけを3つ生成する
- 「あなたは〜と感じたことがありますか？」「あなたにとって〜とはどんな意味ですか？」のような内省を促す問いかけ
- 読んだ後に「よし、明日からこうしよう」と思えるような問いかけ
- リードマネジメント・インサイドアウトの視点で、自律的な行動を引き出す問いかけ

以下の形式で返してください（問いかけ部分のみ。説明不要）：

---

💭 この章を読んで、あなた自身に問いかけてみましょう
