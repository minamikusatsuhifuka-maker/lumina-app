...
export default function TipsSection() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('lumina_tips');
      if (stored) {
        const parsed = JSON.parse(stored);
        setTips([...DEFAULT_TIPS, ...parsed]);
      }
    } catch {}
  }, []);
  ...
  const addTip = () => {
    ...
    const stored = JSON.parse(localStorage.getItem('lumina_tips') || '[]');
    stored.push(tip);
    localStorage.setItem('lumina_tips', JSON.stringify(stored));
