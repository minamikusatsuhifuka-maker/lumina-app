  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        // 270: 読めなかったときの挙動を変更（クリアしない）。案内文もそれに合わせる
        keys: ['⌘', '⇧', 'V'],
...
//
// 270: ⌘⇧V の割り当ては変えない（3キーの例外はU15の表に登録済み＝R-60）。
//   変えたのは**結末**だけで、クリップボードを読めなかったときに入力を消さなくなった
//   （lib/clear-and-paste.ts・R-76）。📝テキスト分析は全端末で3ボタンになったが、
//   キー併記は従来どおりキーボードのある環境だけに出す（useRunKeyHints）
//   ＝ iPhoneのボタンには「⌘⇧V」と書かない（押せないキーを案内しない）。
// ─────────────────────────────────────────────────────────────
