...
// 281: 'episode'（episode_records・整数ID）を追加。名前空間は「ep-N」（ana-N と同じ流儀）。
export type KindleMaterialSource = 'deepresearch' | 'note-article' | 'analysis' | 'episode';
...
export const KINDLE_SOURCE_TABS: KindleMaterialSource[] = ['deepresearch', 'note-article', 'analysis', 'episode'];
...
export const KINDLE_MATERIAL_SOURCE_META: Record<KindleMaterialSource, { emoji: string; label: string }> = {
  ...
  // 281: 著者の実体験（一次情報）。本文はサーバ側で episodeToText に整形して注入する
  episode: { emoji: '📔', label: 'エピソード記録' },
};
...
// 281: 'ep-N' = episode_records（エピソード記録）
const EP_KEY_RE = /^ep-(\d+)$/;

export function makeAnalysisSourceKey(id: number): string {
  return `ana-${id}`;
}

export function makeEpisodeSourceKey(id: number): string {
  return `ep-${id}`;
}

export function parseKindleSourceKey(
  key: string,
): { kind: 'analysis'; id: number } | { kind: 'episode'; id: number } | { kind: 'library'; id: string } {
  const m = ANA_KEY_RE.exec(key);
  if (m) {
    const id = Number(m[1]);
    if (Number.isInteger(id) && id > 0) return { kind: 'analysis', id };
  }
  const e = EP_KEY_RE.exec(key);
  if (e) {
    const id = Number(e[1]);
    if (Number.isInteger(id) && id > 0) return { kind: 'episode', id };
  }
  return { kind: 'library', id: key };
}

export function isEpisodeSourceKey(key: string): boolean {
  return parseKindleSourceKey(key).kind === 'episode';
}
