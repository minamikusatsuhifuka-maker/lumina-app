...
// ③ リードマネジメント哲学（ハードコード）
const leadManagementBlock = `【リードマネジメント哲学】
インサイドアウト（内面から外面へ）の原則で関わる。
5大欲求（生存・愛所属・力・自由・楽しみ）を理解し、本人の上質世界を尊重する。
ボスマネジメント（命令・管理）ではなくリードマネジメント（支援・信頼）で育成する。
フィードバックなくして成長なし。事実を伝え、自己評価を促す。`;
