...
// 229B: Kindle章→note記事への改編指示（/api/kindle/to-note 専用）。
// 書籍の1章を「単体で読み切れる記事」に再構成させる（章の転載を防ぐ）。
export const KINDLE_TO_NOTE_RULES = `# 書籍の章→note記事への改編（厳守）
- 書籍の1章を、その記事だけで読み切れる独立した記事に再構成する（章の文章をそのまま写さない）
- 本の前後章への参照（「前章で述べた」「次章では」等）と書籍特有の言い回しを除去する
- 「この章でわかること」「この章のまとめ」の枠は使わず、内容は記事の導入・結びに自然に溶かす
- 記事タイトルは読者の興味を引く30〜40字で新規に作る（章タイトルの転用でなく記事として立つもの）
- 導入は読者の悩み・関心への語りかけから入る（書籍の文脈説明から入らない）`;
