...
// プロンプトに差し込む NG 表現の列挙。
// 先頭5類型は reply-draft 実装時からの文言を変更しない（既存の検出精度を落とさない）。
// 186: 184で欠落が判明した3観点（不安煽り・限定性・費用誤認）を追加し、hp-guard と基準を統一。
// 誤検知抑制のため、線引きが難しい観点には「該当しない例」を明示する（特に不安煽り＝正当な注意喚起は適正）。
export const MEDICAL_AD_NG_RULES = `- 効果・効能の保証（「必ず治る」「絶対」「効果を保証」等）
- 誇大・最上級（「日本一」「最高」「No.1」根拠なし）
- ビフォーアフターや他患者の体験談的表現の誘導
- 割引・キャンペーンなどの利益誘導
- 未承認・自由診療の不適切な効果訴求
- 不安を煽る表現（該当例:「放置すると危険」「手遅れになる前に」「〜しないと悪化します」等の脅し文句。該当しない例:「早期の受診をおすすめします」「気になる症状があればご相談ください」等の正当な注意喚起・受診の目安の案内は適正）
- 限定性・希少性の演出（該当例:「今だけ」「先着◯名」「期間限定」「キャンペーン終了間近」等）
- 費用に関する誤認を招く表現（該当例:「実質無料」・総額が分からない料金表示・追加費用を伏せた表示等。該当しない例:「保険適用の範囲は診察時にご説明します」等の事実の案内は適正）`;
