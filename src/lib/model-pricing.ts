...
//   Gemini Flash は 2026/12/31 まで $0.75/$3.75、2027/1/1 から $1.50/$7.50（切り替えの判定は JST・R-86）。
//   335: 3.7 Flash → 3.8 Flash に替えたが**単価は同額**。単価表は GEMINI_TEXT_MODEL から引くので値の変更は不要（確認日だけ更新）。
//   335の実測（2026-09-16・3,000字級・medium）: 思考トークンは 3.8 が 744／3.7 が 1207 で**増えていない**
//   （出力の 0.20 倍）。「3.8 は思考を多く使う」は短いお題でだけ再現したため、目安に係数は掛けない（据え置き）。
...
/** 単価を確認した日（画面に添える） */
export const PRICING_CHECKED_ON = '2026-09-16';
